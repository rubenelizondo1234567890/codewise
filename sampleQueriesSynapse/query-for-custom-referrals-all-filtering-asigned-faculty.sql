select SQL_CALC_FOUND_ROWS p.id , p.id, p.firstname, p.lastname, p.risk_level, rml.risk_model_id, ops.status as student_status, il.image_name as intent_imagename,il.text as intent_text, rl.image_name as risk_imagename, rl.risk_text as risk_text, (count(distinct (lc.id))) as login_cnt, p.cohert, p.last_activity, r.id as rid, p.userName as primary_email, pem.metadata_value as class_level, pem.metadata_value as class_level  from person p
 ...
 /* Select students having atleast one referral in specified status */
            ...
            where
                r.organization_id = 20
                 AND r.status in ('O','C') 
                AND (r.person_id_assigned_to = 231679 OR rip.person_id = 231679)
        
)   and r.status in ('O','C') and p.deleted_at is null
                                and r.deleted_at is null  group by (p.id) order by p.risk_level asc ,p.lastname,p.firstname LIMIT 0 , 25;