...
from 
	(
		(SELECT A.id AS AppointmentId,
            ...
            WHERE AL.person_id_student = 1032150 
            AND AL.organization_id = 20 
            AND AL.activity_type IN ("R","N","C","A","E") 
			...
			AND AL.id NOT IN 
				(SELECT ALOG.id 
					FROM related_activities AS related 
                    LEFT JOIN activity_log AS ALOG ON related.appointment_id = ALOG.appointments_id 
                    WHERE related.appointment_id IS NOT NULL 
                    AND related.deleted_at IS NULL 
                    AND ALOG.deleted_at IS NULL
				) 
			AND 
				(CASE 
					WHEN AL.activity_type = "N" THEN 
					CASE WHEN N.access_team = 1 THEN NT.teams_id IN 
						(SELECT teams_id 
							FROM team_members 
                            WHERE person_id = 231679 
                            AND teams_id IN 
								(SELECT teams_id 
									FROM note_teams 
                                    WHERE note_id = N.id 
					 ...
					 AND 1 = 1 ELSE 
				  CASE 
					 WHEN N.access_private = 1 THEN 
                     N.person_id_faculty = 231679 ELSE 
                     N.access_public = 1 
                     AND 1 = 1 
				  END 
                  END 
			 OR N.person_id_faculty = 231679 ELSE CASE WHEN AL.activity_type = "C" THEN CASE WHEN C.access_team = 1 THEN CT.teams_id IN (SELECT teams_id FROM team_members WHERE person_id = 231679 AND teams_id IN (SELECT teams_id FROM contacts_teams WHERE contacts_id = C.id AND deleted_at IS NULL)) AND 1 = 1 ELSE CASE WHEN C.access_private = 1 THEN C.person_id_faculty = 231679 ELSE C.access_public = 1 AND 1 = 1 END END OR C.person_id_faculty = 231679 ELSE CASE WHEN AL.activity_type = "E" THEN CASE WHEN E.access_team = 1 THEN ET.teams_id IN (SELECT teams_id FROM team_members WHERE person_id = 231679 AND teams_id IN (SELECT teams_id FROM email_teams WHERE email_id = E.id AND deleted_at IS NULL)) AND 1 = 1 ELSE CASE WHEN E.access_private = 1 THEN E.person_id_faculty = 231679 ELSE E.access_public = 1 AND 1 = 1 END END OR E.person_id_faculty = 231679 ELSE CASE WHEN AL.activity_type = "R" THEN CASE WHEN R.access_team = 1 THEN RT.teams_id IN (SELECT teams_id FROM team_members WHERE person_id = 231679 AND teams_id IN (SELECT teams_id FROM referrals_teams WHERE referrals_id = R.id AND deleted_at IS NULL)) AND 1 = 1 ELSE CASE WHEN R.access_private = 1 THEN R.person_id_faculty = 231679 ELSE R.access_public = 1 AND 1 = 1 END END OR R.person_id_assigned_to = 231679 OR R.person_id_faculty = 231679 OR orgr.person_id = 231679 and R.person_id_assigned_to is null AND orgr.role_id IN (1,2,3) AND (rr.is_primary_coordinator = 1 AND rr.person_id IS NULL) ELSE CASE WHEN AL.activity_type = "A" THEN CASE WHEN A.access_team = 1 THEN APT.teams_id IN (SELECT teams_id FROM team_members WHERE person_id = 231679 AND teams_id IN (SELECT teams_id FROM appointments_teams WHERE appointments_id = A.id AND deleted_at IS NULL)) AND 1 = 1 ELSE CASE WHEN A.access_private = 1 THEN A.person_id = 231679 ELSE A.access_public = 1 AND 1 = 1 END END ELSE 1 = 1 END END END END END) GROUP BY AL.id) UNION ALL (SELECT null, null, R.id AS ReferralId, null, AL.id AS activity_log_id, AL.created_at AS activity_date, AL.activity_type AS activity_type, AL.person_id_faculty AS activity_created_by_id, P.firstname AS activity_created_by_first_name, P.lastname AS activity_created_by_last_name, AC.id AS activity_reason_id, AC.short_name AS activity_reason_text, null, null, R.status AS activity_referral_status, null, R.note AS referralDescription, null, null, AL.created_at as created_date, null, null, null, null, null, AL.activity_date as act_date FROM activity_log AS AL LEFT JOIN referrals AS R ON AL.referrals_id = R.id LEFT JOIN referrals_teams AS RT ON R.id = RT.referrals_id LEFT JOIN activity_category AS AC ON R.activity_category_id = AC.id LEFT JOIN person AS P ON AL.person_id_faculty = P.id LEFT JOIN referrals_interested_parties as rip ON rip.person_id = 231679 and R.id = rip.referrals_id where rip.person_id = 231679 and R.person_id_student = 1032150 and rip.deleted_at is null)) merger group by activity_log_id ORDER BY act_date DESC;